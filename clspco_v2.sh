...
while getopts ":m:p:b:c:f:s:r:x:h" opt; do
  ...
  ;;
    h)
      ...
      echo " [-x <raw reads without trimming or de-replication (y/n>, default: n]"
      echo " [-r <Remove intermediate files (y/n)>, default: y]"
      echo " [-m <fraction of memory used by spades (0.1-1.0 step 0.1)>, default: 0.8]"
      echo " [-p <nr of threads used by spades>, default: all threads available]"
