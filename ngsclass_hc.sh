...
# ==============================================================================
# NGSCLASS_HC.SH (v1.9.0 - High Coverage Auto-Normalization Version)
# ==============================================================================
...
Help() {
    ...
    echo "Options:"
    echo "  -x [y|n]    Dereplication mode (skip assembly/normalization). Default: $RAW_MODE"
...
NORM_DIR="./normalized"; TRIM_DIR="./trimmed"; ASM_DIR="./assembly"; DERE_DIR="./dereplicated"; COV_DIR="./coverage"
mkdir -p "$ASM_DIR" "$DMND_DIR" "$COV_DIR" "./logs"
[[ "$RAW_MODE" == "n" ]] && mkdir -p "$NORM_DIR" "$TRIM_DIR" || mkdir -p "$DERE_DIR"

# ==============================================================================
# SECTION 1: NORMALIZATION & TRIMMING OR DEREPLICATION
# ==============================================================================
if [[ "$RAW_MODE" == "n" ]]; then
    
    # --- STEP 1A: BBTools Digital Normalization ---
    echo "📊 Mode: High-Coverage Normalization (BBTools bbnorm)"
    for f1 in $INPUT_PATTERN; do
        [ -e "$f1" ] || continue
        f2="${f1/_R1/_R2}"
        sample_name=$(basename "$f1" | sed 's/_R1.*//')
        out_n="$NORM_DIR/${sample_name}"
        
        if [[ ! -f "${out_n}_R1_norm.fastq.gz" ]]; then
            echo "   Normalizing: $sample_name (Target: ${NORM_TARGET}x, Min: ${NORM_MIN})"
            bbnorm.sh \
                in1="$f1" \
                in2="$f2" \
                out1="${out_n}_R1_norm.fastq.gz" \
                out2="${out_n}_R2_norm.fastq.gz" \
                target="$NORM_TARGET" \
                min="$NORM_MIN" \
		bits="$NORM_BITS" \
                unpigz=t \
                &> "./logs/${sample_name}_bbnorm.log"
        fi
    done

    # --- STEP 1B: Trimming (Using Normalized Reads as Input) ---
    echo "✂️ Mode: Trimming (Preparing Normalized Reads for Assembly)"
    for f1_norm in "$NORM_DIR"/*_R1_norm.fastq.gz; do
        [ -e "$f1_norm" ] || continue
        f2_norm="${f1_norm/_R1_norm/_R2_norm}"
        sample_name=$(basename "$f1_norm" | sed 's/_R1_norm.*//')
        ...
        if [[ ! -f "${out_p}_1P.fastq.gz" ]]; then
            ...
            trimmomatic PE -threads "$TRIM_THREADS" -phred33 -quiet \
                -summary "./logs/${sample_name}"_trimmed.log "$f1_norm" "$f2_norm" \
...
for f in "${INPUTS[@]}"; do
    ...
    if [[ ! -f "$tsv_out" && ! -f "${tsv_out}.gz" ]]; then
        ...
        # Catch-all for 'Other'
        awk -v FS='\t' '!($6 ~ /Viruses|Eukaryota|Bacteria|Archaea/) { gsub(/ /,"_"); print ">"$1":"$5"\n"$2 }' "$tsv_out" \
...
if [[ "$REMOVE_TEMP" == "y" ]]; then
    echo "🧹 Cleanup active..."
    [[ "$RAW_MODE" == "n" ]] && rm -rf "$NORM_DIR" "$TRIM_DIR" "$ASM_DIR" || rm -rf "$DERE_DIR"
fi

echo "✅ ngsclass_hc.sh Complete."
