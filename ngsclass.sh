...
Help() {
    ...
    echo "  -f [str]    SPAdes mode flags (--isolate, --sc, --plasmid, \
          --meta, --metaplasmid, --metaviral, --rna, --rnaviral, --bio, \
          --corona, --sewage). Default: $SPADES_FLAGS"
...
# --- 3. Directory Setup ---
TRIM_DIR="./trimmed"
ASM_DIR="./assembly" # Renamed for tool neutrality
...
if [[ "$RAW_MODE" == "n" ]]; then
    ...
    echo "🌀 Mode: Raw Read Dereplication (Streaming zcat -> USEARCH)"
    for f1 in ./fa/*_R1_*.fastq.gz; do
        [ -e "$f1" ] || continue
        sample_name=$(basename "$f1" | sed 's/_L.*//')
...
# ==============================================================================
# SECTION 2: ASSEMBLY (Only if RAW_MODE=n)
# ==============================================================================
if [[ "$RAW_MODE" == "n" ]]; then
...
if [[ "$RAW_MODE" == "y" ]]; then
    ...
    # Automatically pick the right contig file based on assembly tool
...
for f in "${INPUTS[@]}"; do
    ...
    if [[ ! -f "$tsv_out" && ! -f "${tsv_out}.gz" ]]; then
        ...
        diamond blastx -d "$DIAMOND_DB" -q "$f" -o "$tsv_out" \
            --max-target-seqs 1 --evalue 1E-5 -b "$DIAMOND_BLOCK" -c "$DIAMOND_CHUNKS" -t "$THREADS" \
            --outfmt 6 qseqid full_qseq evalue staxids sscinames sskingdoms
